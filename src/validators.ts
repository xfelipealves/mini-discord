...
export function validateConsistency(consistency?: string): { 
  ...
  if (!consistency) {
    const defaultValue = types.consistencies[defaultConsistency.toLowerCase() as keyof typeof types.consistencies];
  ...
  const consistencyValue = types.consistencies[consistency.toLowerCase() as keyof typeof types.consistencies];
  return { value: consistencyValue };
}

export function validatePostMessage(body: any): {
  valid: boolean;
  errors: string[];
  data?: {
    channel_id: string;
...
export function validateGetMessages(query: any): {
  valid: boolean;
  errors: string[];
  data?: {
    limit: number;
